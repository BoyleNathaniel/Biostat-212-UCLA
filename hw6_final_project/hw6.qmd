...
# Convert the hclust object into a data frame suitable for ggplot
dendro_data <- dendro_data(hclust_res)

# Create the dendrogram plot using ggplot2 with expanded X-axis
ggplot(segment(dendro_data), aes(x, y, xend = xend, yend = yend)) +
  geom_segment(size = 1, color = "darkblue") +
  theme_minimal() +
  labs(title = "Hierarchical Clustering Dendrogram", x = "Samples", y = "Height") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8),  # Rotate x-axis labels for clarity
        axis.ticks.x = element_blank(),  # Remove x-axis ticks
        plot.margin = margin(1, 1, 1, 3, "cm")) +  # Increase right margin to allow for space
  scale_x_continuous(expand = c(0.05, 0.05))  # Adjust spacing between branches
```
